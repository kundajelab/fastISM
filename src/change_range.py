...
def not_supported_error(message):
    raise NotImplementedError("{} not supported yet.".format(message))
...
class ChangeRangesBase():
    ...
    def forward_compose(change_ranges_objects_list, input_seqlen, input_change_ranges):
        ...
        if len(change_ranges_objects_list) == 0:
            return input_change_ranges, (0,0), [0]*len(input_change_ranges), input_seqlen, input_change_ranges
        ...
        for change_range_object in change_ranges_objects_list:
            input_range_corrected, _,  _, seqlen, affected_range = change_range_object.forward(
        ...
        initial_input_range_corrected, initial_padding, \
            initial_offsets, _, _ = change_ranges_objects_list[0].forward(
        ...
        # compute new offsets
        offsets=[o+(x_old-x_new) for (x_new, _), (x_old, _), o in zip(
            input_range_corrected, initial_input_range_corrected, initial_offsets)]

        return input_range_corrected, initial_padding, offsets, seqlen, affected_range
